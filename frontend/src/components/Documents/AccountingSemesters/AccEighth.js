...
function AccEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch(
      "https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords24.json",
      {
        method: "POST",
        headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/8/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords24");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting Eighth semester data</h1>
