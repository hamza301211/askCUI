...
function AccFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords21.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/5/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords21");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting Fifth semester data</h1>
