...
function AccFirst() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords17.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/1/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/1/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords17");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting First semester data</h1>
