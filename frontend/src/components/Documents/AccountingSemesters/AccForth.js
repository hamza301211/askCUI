...
function AccForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords20.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/4/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords20");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting Forth semester data</h1>
