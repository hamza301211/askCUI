...
function AccSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords23.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords23");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting Seventh semester data</h1>
