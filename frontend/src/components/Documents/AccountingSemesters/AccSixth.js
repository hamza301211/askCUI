...
function AccSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords22.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/6/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords22");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting Sixth semester data</h1>
