...
function AccThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords19.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "acc/3/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `acc/3/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords19");
  ...
  return (
    <>
      <h1 className="Header">This is Accounting Third semester data</h1>
