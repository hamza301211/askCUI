...
function ArchitForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords28.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "archit/4/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `archit/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords28");
  ...
  return (
    <>
      <h1 className="Header">This is Architecture Forth semester data</h1>
