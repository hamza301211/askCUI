...
function ArchitSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords26.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "archit/2/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `archit/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords26");
  ...
  return (
    <>
      <h1 className="Header">This is Architecture Second semester data</h1>
