...
function ArchitSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords31.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "archit/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `archit/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords31");
  ...
  return (
    <>
      <h1 className="Header">This is Architecture Seventh semester data</h1>
