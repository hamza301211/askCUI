...
function ArchitSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords30.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "archit/6/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `archit/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords30");
  ...
  return (
    <>
      <h1 className="Header">This is Architecture Sixth semester data</h1>
