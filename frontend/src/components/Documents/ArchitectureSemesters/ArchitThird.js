...
function ArchitThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords27.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "archit/3/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `archit/3/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords27");
  ...
  return (
    <>
      <h1 className="Header">This is Architecture Third semester data</h1>
