...
function BusinEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords40.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "busin/8/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `busin/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords40");
  ...
  return (
    <>
      <h1 className="Header">This is Business Eighth semester data</h1>
