...
function BusinFirst() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords33.json", {
      method: "POST",
      headers: {
...
swal("IMAGE UPLOADED !!", " ", "warning", {
  ...
  const imageListRef = ref_storage(storage, "busin/1/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `busin/1/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords33");
  ...
  return (
    <>
      <h1 className="Header">This is Business First semester data</h1>
