...
function BusinForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords36.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "busin/4/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `busin/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords36");
  ...
  return (
    <>
      <h1 className="Header">This is Business Forth semester data</h1>
