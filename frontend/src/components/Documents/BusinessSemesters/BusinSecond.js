...
function BusinSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords34.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "busin/2/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `busin/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords34");
  ...
  return (
    <>
      <h1 className="Header">This is Business Second semester data</h1>
