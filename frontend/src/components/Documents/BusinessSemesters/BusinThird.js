...
function BusinThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords35.json", {
      method: "POST",
      headers: {
...
swal("IMAGE UPLOADED !!", " ", "warning", {
  ...
  const imageListRef = ref_storage(storage, "busin/3/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `busin/3/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords35");
  ...
  return (
    <>
      <h1 className="Header">This is Business Third semester data</h1>
