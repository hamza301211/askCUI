...
function CsFirst() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch(
      "https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords.json",
      {
        method: "POST",
        headers: {
  ...
  const imageListRef = ref_storage(storage, "cs/1./");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `cs/1./${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords");
  ...
  return (
    <>
      <h1 className="Header">This is CS First semester data</h1>
