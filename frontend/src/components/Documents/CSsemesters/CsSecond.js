...
function CsSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords1.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "cs/2/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `cs/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords1");
  ...
  return (
    <>
      <h1 className="Header">This is CS Second semester data</h1>
