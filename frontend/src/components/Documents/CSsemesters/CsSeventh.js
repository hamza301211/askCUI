...
function CsSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords7.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "cs/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `cs/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords7");
  ...
  return (
    <>
      <h1 className="Header">This is CS Seventh semester data</h1>
