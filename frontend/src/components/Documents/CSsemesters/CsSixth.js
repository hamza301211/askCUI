...
function CsSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords6.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "cs/6/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `cs/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords6");
  ...
  return (
    <>
      <h1 className="Header">This is CS Sixth semester data</h1>
