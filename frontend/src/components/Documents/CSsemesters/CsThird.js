...
function CsThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords3.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "cs/3/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `cs/3/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords3");
  ...
  return (
    <>
      <h1 className="Header">This is CS Third semester data</h1>
