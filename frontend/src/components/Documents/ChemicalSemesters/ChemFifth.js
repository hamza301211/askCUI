...
function ChemFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords13.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "chem/5/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `chem/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords13");
  ...
  return (
    <>
      <h1 className="Header">This is Chemical Fifth semester data</h1>
