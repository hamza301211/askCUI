...
function ChemForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords12.json", {
      method: "POST",
      headers: {
...
alert("DATA STORED");
  ...
  const imageListRef = ref_storage(storage, "chem/4/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `chem/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords11");
  ...
  return (
    <>
      <h1 className="Header">This is Chemical Forth semester data</h1>
