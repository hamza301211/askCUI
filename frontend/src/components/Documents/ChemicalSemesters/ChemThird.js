...
function ChemThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords11.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "chem/3/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `chem/3/${imageUpload.name + v4()}`);
  ...
  return (
    <>
      <h1 className="Header">This is Chemical Third semester data</h1>
