...
function ElectEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords48.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "elect/8/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `elect/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords48");
  ...
  return (
    <>
      <h1 className="Header">This is Electrical Eighth semester data</h1>
