...
function ElectFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords45.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "elect/5/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `elect/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords45");
  ...
  return (
    <>
      <h1 className="Header">This is Electrical Fifth semester data</h1>
