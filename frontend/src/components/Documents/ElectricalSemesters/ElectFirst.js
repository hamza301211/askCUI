...
function ElectFirst() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords41.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "elect/1/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `elect/1/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords41");
  ...
  return (
    <>
      <h1 className="Header">This is Electrical First semester data</h1>
