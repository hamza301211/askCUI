...
function ElectForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords44.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "elect/4/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `elect/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords44");
  ...
  return (
    <>
      <h1 className="Header">This is Electrical Forth semester data</h1>
