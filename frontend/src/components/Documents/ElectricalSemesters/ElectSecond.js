...
function ElectSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords42.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "elect/2/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `elect/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords42");
  ...
  return (
    <>
      <h1 className="Header">This is Electrical Second semester data</h1>
