...
function ElectSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords47.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "elect/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `elect/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords47");
  ...
  return (
    <>
      <h1 className="Header">This is Electrical Seventh semester data</h1>
