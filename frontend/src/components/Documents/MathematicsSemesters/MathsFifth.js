...
function MathsFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords53.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "math/5/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `math/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords53");
  ...
  return (
    <>
      <h1 className="Header">This is Mathematics fifth semester data</h1>
