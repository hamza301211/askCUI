...
function MathsForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords52.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "math/4/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `math/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords52");
  ...
  return (
    <>
      <h1 className="Header">This is Mathematics forth semester data</h1>
