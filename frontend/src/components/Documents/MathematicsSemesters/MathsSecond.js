...
function MathsSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords50.json", {
      method: "POST",
      headers: {
...
swal("IMAGE UPLOADED !!", " ", "warning", {
  ...
  const imageListRef = ref_storage(storage, "math/2/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `math/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords50");
  ...
  return (
    <>
      <h1 className="Header">This is Mathematics second semester data</h1>
