...
function MathsSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords55.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "math/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `math/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords55");
  ...
  return (
    <>
      <h1 className="Header">This is Mathematics Seventh semester data</h1>
