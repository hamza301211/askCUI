...
function MathsSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords54.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "math/6/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `math/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords54");
  ...
  return (
    <>
      <h1 className="Header">This is Mathematics Sixth semester data</h1>
