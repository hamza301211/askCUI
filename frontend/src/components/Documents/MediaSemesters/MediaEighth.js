...
function MediaEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords64.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "media/8/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `media/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords64");
  ...
  return (
    <>
      <h1 className="Header">This is Media eighth semester data</h1>
