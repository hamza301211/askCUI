...
function MediaFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords61.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "media/5/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `media/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords61");
  ...
  return (
    <>
      <h1 className="Header">This is Media fifth semester data</h1>
