...
function MediaForth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords60.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "media/4/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `media/4/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords60");
  ...
  return (
    <>
      <h1 className="Header">This is Media forth semester data</h1>
