...
function MediaSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords58.json", {
      method: "POST",
      headers: {
...
swal("IMAGE UPLOADED !!", " ", "warning", {
  ...
  const imageListRef = ref_storage(storage, "media/2/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `media/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords58");
  ...
  return (
    <>
      <h1 className="Header">This is Media second semester data</h1>
