...
function MediaSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords63.json", {
      method: "POST",
      headers: {
...
swal("IMAGE UPLOADED !!", " ", "warning", {
  ...
  const imageListRef = ref_storage(storage, "media/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `media/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords63");
  ...
  return (
    <>
      <h1 className="Header">This is Media seventh semester data</h1>
