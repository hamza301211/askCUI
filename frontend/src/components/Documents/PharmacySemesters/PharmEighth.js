...
function PharmEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords72.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "pharm/8/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `pharm/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords72");
  ...
  return (
    <>
      <h1 className="Header">This is Pharmacy eighth semester data</h1>
