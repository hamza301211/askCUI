...
function PharmFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords69.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "pharm/5/");
...
history.push("/auth");}
    ...
    const imageRef = ref_storage(storage, `pharm/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords69");
  ...
  return (
    <>
      <h1 className="Header">This is Pharmacy fifth semester data</h1>
