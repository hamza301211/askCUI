...
function PharmSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords66.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "pharm/2/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `pharm/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords66");
  ...
  return (
    <>
      <h1 className="Header">This is Pharmacy second semester data</h1>
