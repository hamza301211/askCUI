...
function PharmSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords71.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "pharm/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `pharm/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords71");
  ...
  return (
    <>
      <h1 className="Header">This is Pharmacy seventh semester data</h1>
