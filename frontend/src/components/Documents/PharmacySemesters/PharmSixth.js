...
function PharmSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords70.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "pharm/6/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `pharm/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords70");
  ...
  return (
    <>
      <h1 className="Header">This is Pharmacy sixth semester data</h1>
