...
function PharmThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords67.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "pharm/3/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `pharm/3/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords67");
  ...
  return (
    <>
      <h1 className="Header">This is Pharmacy third semester data</h1>
