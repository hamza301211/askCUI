...
function PhysEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords80.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "phys/8/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `phys/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords80");
  ...
  return (
    <>
      <h1 className="Header">This is Physics eighth semester data</h1>
