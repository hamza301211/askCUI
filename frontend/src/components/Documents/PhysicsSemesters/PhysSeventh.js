...
function PhysSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords79.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "phys/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `phys/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords79");
  ...
  return (
    <>
      <h1 className="Header">This is Physics seventh semester data</h1>
