...
function PhysSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords78.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "phys/6/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `phys/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords78");
  ...
  return (
    <>
      <h1 className="Header">This is Physics sixth semester data</h1>
