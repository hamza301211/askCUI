...
function PsychEighth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords88.json", {
      method: "POST",
      headers: {
...
swal("IMAGE UPLOADED !!", " ", "warning", {
  ...
  const imageListRef = ref_storage(storage, "psych/8/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/8/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords88");
  ...
  return (
    <>
      <h1 className="Header">This is psychology eighth semester data</h1>
