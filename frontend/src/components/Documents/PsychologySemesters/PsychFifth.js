...
function PsychFifth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords85.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "psych/5/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/5/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords85");
  ...
  return (
    <>
      <h1 className="Header">This is psychology fifth semester data</h1>
