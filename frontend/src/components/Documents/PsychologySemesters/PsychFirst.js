...
function PsychFirst() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords81.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "psych/1/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/1/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords81");
  ...
  return (
    <>
      <h1 className="Header">This is psychology first semester data</h1>
