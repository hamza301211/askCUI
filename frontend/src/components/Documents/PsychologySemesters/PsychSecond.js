...
function PsychSecond() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords82.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "psych/2/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/2/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords82");
  ...
  return (
    <>
      <h1 className="Header">This is psychology second semester data</h1>
