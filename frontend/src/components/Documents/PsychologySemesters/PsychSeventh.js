...
function PsychSeventh() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords87.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "psych/7/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/7/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords87");
  ...
  return (
    <>
      <h1 className="Header">This is psychology seventh semester data</h1>
