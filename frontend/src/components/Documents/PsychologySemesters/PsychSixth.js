...
function PsychSixth() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords86.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "psych/6/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/6/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords86");
  ...
  return (
    <>
      <h1 className="Header">This is psychology sixth semester data</h1>
