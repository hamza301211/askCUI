...
function PsychThird() {
  ...
  const submitData = async (event) => {
    ...
    const res = await fetch("https://askcui-985f2-default-rtdb.firebaseio.com/userDataRecords83.json", {
      method: "POST",
      headers: {
  ...
  const imageListRef = ref_storage(storage, "psych/3/");
  ...
  const uploadImage = () => {
    ...
    const imageRef = ref_storage(storage, `psych/3/${imageUpload.name + v4()}`);
  ...
  useEffect(() => {
    const dbref = ref_database(db, "userDataRecords83");
  ...
  return (
    <>
      <h1 className="Header">This is psychology third semester data</h1>
